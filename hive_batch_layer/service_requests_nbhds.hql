create external table cloftus_service_requests_nbhds (
  commarea_year string, 
  street_lights bigint,
  rodents bigint, graffiti bigint,
  potholes bigint, sanitation_codes bigint)
STORED BY 'org.apache.hadoop.hive.hbase.HBaseStorageHandler'
WITH SERDEPROPERTIES ('hbase.columns.mapping' = ':key,request:street_lights,request:rodents,request:graffiti,request:potholes,request:sanitation_codes')
TBLPROPERTIES ('hbase.table.name' = 'cloftus_service_requests_nbhds');

insert overwrite table cloftus_service_requests_nbhds
...
create external table cloftus_neighbourhoods (
  community string, 
  commarea string)
STORED BY 'org.apache.hadoop.hive.hbase.HBaseStorageHandler'
WITH SERDEPROPERTIES ('hbase.columns.mapping' = ':key,nbhd:commarea')
TBLPROPERTIES ('hbase.table.name' = 'cloftus_neighbourhoods');

insert overwrite table cloftus_neighbourhoods
  select distinct community, lpad(area_number, 2, "0") as commarea
  from cloftus_chicago_nbhds;
